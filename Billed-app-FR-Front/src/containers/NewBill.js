...
export default class NewBill {
	...
	handleChangeFile = (e) => {
		...
		if (!allowedMimeTypes.includes(file.type)) {
			...
			if (!errorMessageElement) {
				...
				fileInputElement.before(errorMessageElement);
			}
			errorMessageElement.textContent =
				"Ce type de fichier n'est pas autorisé. Veuillez sélectionner un fichier au format jpg, jpeg ou png.";
			fileInputElement.value = '';
			return;
		} else {
			// Remove the error message if the MIME type is valid
	...
	handleSubmit = (e) => {
		...
		console.log(
			...
			e.target.querySelector(`input[data-testid="datepicker"]`)
				.value,
		);
		const email = JSON.parse(localStorage.getItem('user')).email;
		const bill = {
